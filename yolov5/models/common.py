...
class ASFFLayer(nn.Module):
    def __init__(self,level):
        ...
        self.channel = self.dim[self.level-1]
        if self.level == 1:
            self.sequential13 = nn.Sequential(
                nn.Conv2d(128, self.channel, kernel_size=3, stride=2, padding=1, bias=False),
                nn.BatchNorm2d(self.channel),
                nn.LeakyReLU(0.1),
                nn.MaxPool2d(3, 2, 1)
            )
            self.sequential12 = nn.Sequential(
                nn.Conv2d(256, self.channel, kernel_size=3, stride=2, padding=1, bias=False),
                nn.BatchNorm2d(self.channel),
                nn.LeakyReLU(0.1),
            )

        elif self.level == 2:
            self.sequential23 = nn.Sequential(
                nn.Conv2d(128, self.channel, kernel_size=3, stride=2, padding=1, bias=False),
                nn.BatchNorm2d(self.channel),
                nn.LeakyReLU(0.1),
            )
            self.sequential21 = nn.Sequential(
                nn.Conv2d(512, self.channel, kernel_size=1, stride=1, padding=0, bias=False),
                nn.BatchNorm2d(self.channel),
                nn.LeakyReLU(0.1),
                nn.Upsample(scale_factor=2)
            )
        elif self.level == 3:
            self.sequential31 = nn.Sequential(
                nn.Conv2d(512, self.channel, kernel_size=1, stride=1, padding=0, bias=False),
                nn.BatchNorm2d(self.channel),
                nn.LeakyReLU(0.1),
                nn.Upsample(scale_factor=4)
            )
            self.sequential32 = nn.Sequential(
                nn.Conv2d(256, self.channel, kernel_size=1, stride=1, padding=0, bias=False),
                nn.BatchNorm2d(self.channel),
                nn.LeakyReLU(0.1),
                nn.Upsample(scale_factor=2)
            )
        ...
        self.weight_compress = nn.Sequential(
            nn.Conv2d(16*3, 3, kernel_size=1, stride=1, padding=0, bias=False),
            nn.BatchNorm2d(3),
            nn.LeakyReLU(0.1),
        )
    def forward(self,x_level_1,x_level_2,x_level_3):
        if self.level == 1:
            x_1 = x_level_1
            x_3 = self.sequential13(x_level_3)
            x_2 = self.sequential12(x_level_2)
            weight = F.sigmoid(self.weight_compress(
                torch.cat((self.weight_level_1(x_1), self.weight_level_1(x_2), self.weight_level_1(x_3)), dim=1)))
            out = x_1 * weight[:, 0:1, :, :] + x_2 * weight[:, 1:2, :, :] + x_3 * weight[:,2:3,:,:]
            return out
        elif self.level == 2:
            x_2 = x_level_2
            x_3 = self.sequential23(x_level_3)
            x_1 = self.sequential21(x_level_1)
            weight = F.sigmoid(self.weight_compress(
                torch.cat((self.weight_level_2(x_1), self.weight_level_2(x_2), self.weight_level_2(x_3)), dim=1)))
            out = x_1 * weight[:, 0:1, :, :] + x_2 * weight[:, 1:2, :, :] + x_3 * weight[:,2:3,:,:]
            return out
        elif self.level == 3:
            x_3 = x_level_3
            x_1 = self.sequential31(x_level_1)
            x_2 = self.sequential32(x_level_2)
            weight = F.sigmoid(self.weight_compress(
                torch.cat((self.weight_level_3(x_1), self.weight_level_3(x_2), self.weight_level_3(x_3)), dim=1)))
            out = x_1 * weight[:, 0:1, :, :] + x_2 * weight[:, 1:2, :, :] + x_3 * weight[:,2:3,:,:]
            return out
...
class SELayer(nn.Module):
    ...
    def forward(self,x):
        x_se = F.sigmoid(self.se(F.avg_pool2d(x,x.size(2),x.size(2)))).unsqueeze(2).unsqueeze(3).expand_as(x)
        return x*x_se
class MLFPNLayer(nn.Module):
    def __init__(self):
        super().__init__()
        self.conv = nn.Sequential(
        ...
        self.sfam = nn.Sequential(
            SELayer(3*128,16)
        )

    def forward(self,x_p1,x_p2):
        x_ffm1 = torch.cat((self.conv1(x_p2),self.conv2(x_p1)),dim=1)

        x11 = self.conv4(x_ffm1)
        x21 = self.conv4(x11)
        x31 = self.conv4(x21)
        x41 = self.conv4(x31)
        x51 = self.conv4(x41)
        x_output_61 = self.conv6(x51) # 输出
        x_add_51 = torch.add(F.upsample(self.conv5(x51),scale_factor=2.5),x41)
        x_output_51 = self.conv6(x_add_51)
        x_add_41 = torch.add(F.upsample(self.conv5(x_add_51),scale_factor=2),x31)
        x_output_41 = self.conv6(x_add_41)
        x_add_31 = torch.add(F.upsample(self.conv5(x_add_41),scale_factor=2), x21)
        x_output_31 = self.conv6(x_add_31)
        x_add_21 = torch.add(F.upsample(self.conv5(x_add_31),scale_factor=2), x11)
        x_output_21 = self.conv6(x_add_21)
        x_add_11 = torch.add(F.upsample(self.conv5(x_add_21),scale_factor=2), x_ffm1)
        x_output_11 = self.conv6(x_add_11)


        x_ffm21 = self.conv(torch.cat((x_output_11,x_ffm1),dim=1))

        x12 = self.conv4(x_ffm21)
        x22 = self.conv4(x12)
        x32 = self.conv4(x22)
        x42 = self.conv4(x32)
        x52 = self.conv4(x42)
        x_output_62 = self.conv6(x52)  # 输出
        x_add_52 = torch.add(F.upsample(self.conv5(x52), scale_factor=2.5), x42)
        x_output_52 = self.conv6(x_add_52)
        x_add_42 = torch.add(F.upsample(self.conv5(x_add_52), scale_factor=2), x32)
        x_output_42 = self.conv6(x_add_42)
        x_add_32 = torch.add(F.upsample(self.conv5(x_add_42), scale_factor=2), x22)
        x_output_32 = self.conv6(x_add_32)
        x_add_22 = torch.add(F.upsample(self.conv5(x_add_32), scale_factor=2), x12)
        x_output_22 = self.conv6(x_add_22)
        x_add_12 = torch.add(F.upsample(self.conv5(x_add_22), scale_factor=2), x_ffm21)
        x_output_12 = self.conv6(x_add_12)


        x_ffm22 = self.conv(torch.cat((x_output_12, x_ffm21), dim=1))

        x13 = self.conv4(x_ffm22)
        x23 = self.conv4(x13)
        x33 = self.conv4(x23)
        x43 = self.conv4(x33)
        x53 = self.conv4(x43)
        x_output_63 = self.conv6(x53)  # 输出
        x_add_53 = torch.add(F.upsample(self.conv5(x53), scale_factor=2.5), x43)
        x_output_53 = self.conv6(x_add_53)
        x_add_43 = torch.add(F.upsample(self.conv5(x_add_53), scale_factor=2), x33)
        x_output_43 = self.conv6(x_add_43)
        x_add_33 = torch.add(F.upsample(self.conv5(x_add_43), scale_factor=2), x23)
        x_output_33 = self.conv6(x_add_33)
        x_add_23 = torch.add(F.upsample(self.conv5(x_add_33), scale_factor=2), x13)
        x_output_23 = self.conv6(x_add_23)
        x_add_13 = torch.add(F.upsample(self.conv5(x_add_23), scale_factor=2), x_ffm22)
        x_output_13 = self.conv6(x_add_13)

        output_p1 = self.sfam(torch.cat((x_output_11,x_output_12,x_output_13),dim=1))
        output_p2 = self.sfam(torch.cat((x_output_21,x_output_22,x_output_23),dim=1))
        output_p3 = self.sfam(torch.cat((x_output_31,x_output_32,x_output_33),dim=1))
